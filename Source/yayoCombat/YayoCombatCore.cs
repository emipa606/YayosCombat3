...
namespace yayoCombat
{
    ...
    public static class YayoCombatCore
    {
        ...
        static YayoCombatCore()
        {
            ...
            if (ModsConfig.ActiveModsInLoadOrder.Any(mod => mod.PackageId != null &&
                                                            mod.PackageId.ToLower().Contains("dualwield")))
            {
                using_dualWeld = true;
            }

            if (ModsConfig.ActiveModsInLoadOrder.Any(mod => mod.PackageId != null &&
                                                            mod.PackageId.ToLower()
                                                                .Contains("co.uk.epicguru.meleeanimation")))
            {
                using_meleeAnimations = true;
            }

            if (ModsConfig.ActiveModsInLoadOrder.Any(mod => mod.PackageId != null &&
                                                            mod.PackageId.ToLower().Contains("humanoidalienraces")))
            {
                using_AlienRaces = true;
            }

            if (ModsConfig.ActiveModsInLoadOrder.Any(mod => mod.PackageId != null &&
                                                            mod.PackageId.ToLower().Contains("showmeyourhands")))
            {
            ...
            if (using_Oversized)
            {
                ...
                try
                {
                    ...
                    foreach (var weapon in allWeapons) saveWeaponOffsets(weapon);
                }
                catch (Exception ex)
                {
        ...
        {
            if (!using_Oversized || pawn == null || weapon == null) return Vector3.zero;
            ...
            try
            {
                ...
                if (handProtect)
                {
                                 ...
                                 !thing.apparel.bodyPartGroups.Contains(yayoCombat_Defs.BodyPartGroupDefOf.Shoulders)))
                    {
                        ...
                        if (list.Count > 0) item.apparel.layers = list;
                    }
                ...
                if (ammo)
                {
                                                                                         ...
                                                                                         "HeavyWeapons.HeavyWeapon"))))
                    {
                        ...
                        if ((int)item3.techLevel <= 1) continue;
                        ...
                        if (verbProperties.defaultProjectile is { projectile.damageDef: not null })
                        {
                            ...
                            {
                                if (item3.weaponTags.Contains("ammo_none")) continue;
                                ...
                                if (containStringByList != "")
                                {
                                    ...
                                    text = $"yy_{array[0]}";
                                    if (ThingDef.Named(text) == null) text = "";

                                    if (array.Length >= 2 && int.TryParse(array[1], out var result))
                                    {
                                        compProperties_Reloadable.maxCharges = Mathf.Max(1,
                            ...
                            if (text == "")
                            {
                                ...
                                if (new List<DamageDef> { DamageDefOf.Bomb, DamageDefOf.Flame, DamageDefOf.Burn }
        ...
        private static bool containCheckByList(string origin, List<string> ar)
        {
            ...
            {
                if (origin.Contains(value)) return true;
            }
        ...
        private static string getContainStringByList(string keyword, List<string> ar)
        {
            ...
            {
                if (containStringByList.Contains(keyword)) return containStringByList;
            }
